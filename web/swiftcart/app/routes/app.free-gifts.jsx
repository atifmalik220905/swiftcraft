...
export const action = async ({ request }) => {
  ...
  switch (intent) {
    ...
    case "delete": {
      await prisma.freeGiftRule.delete({ where: { id: formData.get("giftId") } });
      return { success: true };
    }
    case "toggle": {
      const gift = await prisma.freeGiftRule.findUnique({ where: { id: formData.get("giftId") } });
      if (gift) {
        await prisma.freeGiftRule.update({ where: { id: gift.id }, data: { isActive: !gift.isActive } });
      }
      return { success: true };
    }
    default:
  ...
  return (
    <s-section heading="New Gift Rule">
      <s-stack direction="block" gap="base">
        ...
        >
          Allow customer to choose gift from a collection
        </s-checkbox>
