...
import {
  ...
  getMerchant,
  upsertProgressBarRule,
  deleteProgressBarRule,
  toggleProgressBarRule,
...
export const action = async ({ request }) => {
  ...
  switch (intent) {
    case "upsert": {
      ...
      await upsertProgressBarRule(merchant.id, milestoneOrder, {
    ...
    case "delete": {
      ...
      await deleteProgressBarRule(id);
      result = { success: true };
      break;
    }
    ...
    case "toggle": {
      const id = formData.get("milestoneId");
      await toggleProgressBarRule(id);
      result = { success: true };
      break;
    }
  }
    ...
    <s-section heading={`Milestone ${order}`}>
      <s-stack direction="block" gap="base">
        ...
        <label className="flex items-center gap-3 cursor-pointer select-none py-1.5">
          ...
          />
          <span className="text-body-md text-on-surface font-medium">Animated shimmer effect</span>
        </label>
